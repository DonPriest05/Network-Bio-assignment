...
class Soma:
    ...
    def __init__(self, params, V_soma_init, n_init, h_init, k_init, l_init, q_init):
    ...
    def soma_currents(self, V_soma, V_dend, n, h, k, l, q):
        ...
        I_sd = sum(self.g_sd/self.p * (V_soma - Vd) for Vd in V_dend)
    ...
    def update_gating_variables(self, V_soma, n, h, k, l, q, dt):
        ...
        alpha_n = 0.01 * (V_soma + 55) / (1 - np.exp(-(V_soma + 55) / 10 + 1e-9))
        ...
        h_next = np.clip(h_next, 0, 1)
        k_next = np.clip(k_next, 0, 1)
        l_next = np.clip(l_next, 0, 1)
        q_next = np.clip(q_next, 0, 1)
 
        return n_next, h_next, k_next, l_next, q_next


    def RK_soma(self, V_soma, V_dend, n, h, k, l, q, I, dt):
        def dV_soma_dt(V_soma, n, h, k, l, q):
            I_Ca_soma, I_L_soma, I_h, I_Na_soma, I_K_soma, I_sd = self.soma_currents(V_soma, V_dend, n, h, k, l, q)
            return (- (I_Ca_soma + I_L_soma + I_h + I_Na_soma + I_K_soma + I_sd) + I) / self.Cm_soma

        # k1 values
        k1_v = dV_soma_dt(V_soma, n, h, k, l, q)
        n1, h1, k1, l1, q1 = self.update_gating_variables(V_soma, n, h, k, l, q, dt)

        # k2 values
        k2_v = dV_soma_dt(V_soma + 0.5 * k1_v * dt, n1, h1, k1, l1, q1)
        n2, h2, k2, l2, q2 = self.update_gating_variables(V_soma + 0.5 * k1_v * dt, n1, h1, k1, l1, q1, dt)

        # k3 values
        k3_v = dV_soma_dt(V_soma + 0.5 * k2_v * dt, n2, h2, k2, l2, q2)
        n3, h3, k3, l3, q3 = self.update_gating_variables(V_soma + 0.5 * k2_v * dt, n2, h2, k2, l2, q2, dt)

        # k4 values
        k4_v = dV_soma_dt(V_soma + k3_v * dt, n3, h3, k3, l3, q3)
